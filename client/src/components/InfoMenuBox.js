...
const InfoMenuBox = (props) => {
  const [showControls, setShowControls] = useState(false)
  const [showNotes, setShowNotes] = useState(true)
  const [controlSetToggle1, setControlSetToggle1] = useState(true)
  const [controlSetToggle2, setControlSetToggle2] = useState(true)
  ...
  const toggleControlsHighlight = () => {
    setHighlightControls(!highlightControls);
  }
  ...
  const onClick = (id) => {
    if (id === "keyboard1" || id === "gamepad1") {
      ...
      setControlSetToggle1(!controlSetToggle1);

      props.setControlConfig(id);

    }
    else if (id === "keyboard2" || id === "gamepad2") {
      setControlSetToggle2(!controlSetToggle2);
  ...
  if (controls) {
    ...
    keyBoardButtons = <Grid item xs={2}>
      <ControlsButton id={"keyboard1"} image={KeyBoardIcon} paddingTop={35} onClick={onClick} selected={controlSetToggle1}></ControlsButton>
      <ControlsButton id={"keyboard2"} image={KeyBoardIcon} paddingTop={65} onClick={onClick} selected={controlSetToggle2}></ControlsButton>
    </Grid>;
    gamePadButtons = <Grid item xs={2}>
      <ControlsButton id={"gamepad1"} image={GamePadIcon} paddingTop={33} onClick={onClick} selected={!controlSetToggle1}></ControlsButton>
      <ControlsButton id={"gamepad2"} image={GamePadIcon} paddingTop={62} onClick={onClick} selected={!controlSetToggle2}></ControlsButton>
    </Grid>;
  ...
  return <div style={{ paddingLeft: 20, paddingTop: 10, height: '100%' }}>
    <Grid container>
      <Grid item xs={6}>
        <div onClick={showControlsChange} onMouseEnter={toggleControlsHighlight} onMouseLeave={toggleControlsHighlight} style={{ cursor: 'pointer', borderRadius: 15, backgroundColor: '#9A9A9A', textAlign: 'center', padding: 10, border: controlsHighlightColor }}>
          Controls
                        </div>
      </Grid>
      <Grid item xs={6}>
        <div onClick={showNotesChange} onMouseEnter={toggleNotesHighlight} onMouseLeave={toggleNotesHighlight} style={{ cursor: 'pointer', borderRadius: 15, backgroundColor: '#9A9A9A', textAlign: 'center', padding: 10, border: notesHighlightColor }}>
          Notes
                        </div>
      </Grid>
