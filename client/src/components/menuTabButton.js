...
const MenuTabButton = (props) => {
    const toggle = () => {
        ...
        return <div>
            <div onClick={toggle} className="menuTabButton" style = {{ position: 'relative', textAlign: 'center', margin: 'auto', height: 39.5, backgroundColor: 'grey', width: '70%', cursor: 'pointer', borderRadius: 3, border: ("1px solid "+ highlightColor)}}>
                <img style={{backgroundSize: 'cover', opacity: .3, position: 'relative', width: '100%', top: 0, left: 0}} src={BrickImage} alt="this is car image" />
                {swordMarker}
                <div style={{fontSize: 20, fontFamily: 'Audiowide', position: 'absolute', textAlign: 'center', top: 4, left: 38}}>
